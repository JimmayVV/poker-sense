...
describe("Game Constants & Enums", () => {
  ...
  describe("BLIND_STRUCTURES", () => {
    ...
    it("should have increasing blinds at each level", () => {
      ...
      for (let i = 1; i < structure.length; i++) {
        ...
        expect(current.smallBlind).toBeGreaterThan(previous.smallBlind);
        expect(current.bigBlind).toBeGreaterThan(previous.bigBlind);
      }
    });
    ...
    it("should have turbo structure with shorter durations than standard", () => {
      ...
      const turboFirst = turbo[0];
      if (standardFirst && turboFirst) {
        expect(turboFirst.durationMinutes).toBeLessThan(standardFirst.durationMinutes);
      }
    });
  });
  ...
  describe("TABLE_CONFIG", () => {
    ...
    it("should define 6-max as recommended size", () => {
      ...
      expect(TABLE_CONFIG.RECOMMENDED_PLAYERS).toBeGreaterThanOrEqual(
        TABLE_CONFIG.MIN_PLAYERS
      );
      expect(TABLE_CONFIG.RECOMMENDED_PLAYERS).toBeLessThanOrEqual(
        TABLE_CONFIG.MAX_PLAYERS
      );
    });
  });
  ...
  describe("TOURNAMENT_CONFIG", () => {
    ...
    it("should have payout percentages sum to 100%", () => {
      const sum = TOURNAMENT_CONFIG.PAYOUT_STRUCTURE.reduce(
        (acc, payout) => acc + payout.percentage,
        0
      );
  ...
  describe("CHIP_DENOMINATIONS", () => {
    ...
    it("should have increasing denominations", () => {
      for (let i = 1; i < CHIP_DENOMINATIONS.length; i++) {
        const current = CHIP_DENOMINATIONS[i];
        const previous = CHIP_DENOMINATIONS[i - 1];
  ...
  describe("TIMEOUTS", () => {
    ...
    it("should have disconnect timeout longer than action timeout", () => {
      expect(TIMEOUTS.DISCONNECT_TIMEOUT_SECONDS).toBeGreaterThan(
        TIMEOUTS.ACTION_TIMEOUT_SECONDS
      );
    });
